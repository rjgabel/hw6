...
void HashTable<K, V, Prober, Hash, KEqual>::insert(const ItemType& p) {
    // Division is evil, avoid it. (Okay, maybe it's not evil, but it's still
    // much slower than multiplication)
    if (table_size >= CAPACITIES[mIndex_] * alpha) {
        resize();
    }
    HASH_INDEX_T h = this->probe(p.first);
    if (npos == h) {
        // Couldn't find a free space, but the table isn't full (likely
        // quadratic hashing error)
...
void HashTable<K, V, Prober, Hash, KEqual>::resize() {
    std::vector<HashItem*> items = table_;
    mIndex_++;
    table_.clear();
    ...
    for (size_t i = 0; i < items.size(); i++) {
        if (items[i] != nullptr) {
            if (items[i]->deleted) {
                ...
                HASH_INDEX_T h = probe(items[i]->item.first);
                table_[h] = items[i];
                table_size++;
            }
        }
